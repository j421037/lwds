...
<div class="boardnav cl">
  ...
  <div id="ct" class="wp cl" {if $leftside} style="margin-left:180px"{/if}> 
    ...
    <div id="sd_bdr">
      <div class="forum-bar">
        <dl class="forum-bar-hd">
          <dt class="forum-bar-img"> <a href="forum.php?mod=forumdisplay&fid=$_G[fid]"> 
            <!--{if $_G['forum'][icon]}--> 
            <img src="data/attachment/common/$_G['forum'][icon]" alt="$_G['forum'][name]" width="60" /> 
            <!--{else}--> 
            <img src="static/image/common/forum_new.gif" alt="$_G['forum'][name]" width="60" /> 
            <!--{/if}--> 
            <span class="img-shadow"></span> </a> </dt>
          <dd>
            <h1><a href="forum.php?mod=forumdisplay&fid=$_G[fid]" title="">$_G['forum'][name]</a></h1>
            <p><!--{if rssforumperm($_G['forum']) && $_G[setting][rssstatus] && !$_GET['archiveid'] && !$subforumonly}--><a href="forum.php?mod=rss&fid=$_G[fid]&auth=$rssauth" class="fa_rss" target="_blank" title="RSS">{lang rss_subscribe_this}</a><span class="pipe">|</span><!--{/if}--><a href="home.php?mod=spacecp&ac=favorite&type=forum&id=$_G[fid]&handlekey=favoriteforum" id="a_favorite" onclick="showWindow(this.id, this.href, 'get', 0);">¹Ø×¢ <em class="xi1" id="number_favorite" {if !$_G[forum][favtimes]} style="display:none;"{/if}>(<span id="number_favorite_num">$_G[forum][favtimes]</span>)</em></a></p>
          </dd>
        </dl>
        <div class="forum-rule">
          ...
          <!--{if $moderatedby}-->
          <p class="forum-moderator"> <span>{lang forum_modedby}£º</span> $moderatedby </p>
          <!--{/if}--> 
        </div>
        
        <!--{if !$subforumonly}-->
        <div class="forum-bar-info"> <span><b>$_G[forum][todayposts]</b><strong>{lang index_today}</strong></span> <span class="bni"><b>$_G[forum][threads]</b><strong>{lang index_threads}</strong></span> </div>
        <!--{/if}--> 
      </div>
...
<div class="display-z cl">
    ...
    <div class="mn pa20">
      <div class="bm bml">
        <div class="bm_h cl">
          <div id="pt" class="bm cl z">
            <div class="z"> <a href="./" class="nvhm" title="{lang homepage}">$_G[setting][bbname]</a><em>&raquo;</em><a href="forum.php"{if $_G['setting']['forumjump']} id="fjump" onmouseover="delayShow(this, 'showForummenu($_G[fid])');" class="showmenu" {/if}>{$_G[setting][navs][2][navname]}</a>$navigation </div>
          </div>
      ...
      <!--{if empty($_G['forum']['threadtypes']) || empty($_G['forum']['threadtypes']['listable'])}-->
      <span id="fd_page_top">$multipage</span> <span class="pgb y" {if $_G[setting][visitedforums]}id="visitedforums" onmouseover="$('visitedforums').id = 'visitedforumstmp';this.id = 'visitedforums';showMenu({'ctrlid':this.id,'pos':'34'})"{/if} ><a href="forum.php">{lang return_index}</a></span> 

      
        <!--{/if}-->    
            <div class="npgs {if empty($_G['forum']['threadtypes']) || empty($_G['forum']['threadtypes']['listable'])}z mb10{else} y ml20 {/if}">
