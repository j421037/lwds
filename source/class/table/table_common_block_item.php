...
class table_common_block_item extends discuz_table
{
	...
	public function fetch_all_by_bid($bids, $sort = false) {
        $suffix = $this->checkLanguage();

		return ($bids = dintval($bids, true)) ? DB::fetch_all('SELECT * FROM '.DB::table($this->_table.$suffix).' WHERE '.DB::field('bid', $bids).($sort ? ' ORDER BY displayorder, itemtype DESC' : ''), null, $this->_pk) : array();
	}
	...
	public function insert_batch($bid, $itemlist) {
		...
		if($inserts) {
			DB::query('REPLACE INTO '.DB::table($this->_table.$suffix).'(itemid, bid, itemtype, id, idtype, title, url, pic, picflag, makethumb, thumbpath, summary, showstyle, related, `fields`, displayorder, startdate, enddate) VALUES '.implode(',', $inserts));
		}
	}
	...
	protected function checkLanguage()
    {
        global $_G;
        $suffix = null;
        $language = $_COOKIE["wx_language"];
        $languages = array_keys($_G["config"]["wx"]["language"]);

        if (strtolower($language) != "zh" && in_array($language, $languages)) {
            $suffix = "_".$language;
        }
