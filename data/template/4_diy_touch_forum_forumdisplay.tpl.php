...
<a href="forum.php?mod=forumdisplay&amp;fid=<?php echo $_G['fid'];?><?php if($_G['forum']['threadsorts']['defaultshow']) { ?>&amp;filter=sortall&amp;sortall=1<?php } if($_GET['archiveid']) { ?>&amp;archiveid=<?php echo $_GET['archiveid'];?><?php } ?>">È«²¿·ÖÀà
...
<p>
<?php if(!$subforumonly) { ?>
<span>ä»Šæ—¥: <?php echo $_G['forum']['todayposts'];?></span>
<span>ä¸»é¢˜: <?php echo $_G['forum']['threads'];?></span>
<?php if($_G['forum']['rank']) { ?>
<span title="ä¸Šæ¬¡æ’å:<?php echo $_G['forum']['oldrank'];?>">æ’å: <?php echo $_G['forum']['rank'];?></span>
<?php } } ?>
</p>
<a class="fa_fav" href="home.php?mod=spacecp&amp;ac=favorite&amp;type=forum&amp;id=<?php echo $_G['fid'];?>&amp;handlekey=favoriteforum&amp;formhash=<?php echo FORMHASH;?>" id="a_favorite" onclick="showWindow(this.id, this.href, 'get', 0);">ÊÕ²Ø<strong class="xi1" id="number_favorite" <?php if(!$_G['forum']['favtimes']) { ?> style="display:none;"<?php } ?>>(<span id="number_favorite_num"><?php echo $_G['forum']['favtimes'];?></span>)</strong></a>
</div>

<div class="forumListTab cfix">
<ul>
<li>
<a href="forum.php?mod=forumdisplay&amp;fid=<?php echo $_G['fid'];?>&amp;filter=<?php if($_GET['archiveid']) { ?>&amp;archiveid=<?php echo $_GET['archiveid'];?><?php } ?>" class="<?php if($_GET['filter'] == '') { ?>cur<?php } ?>">È«²¿</a>
</li>
<li>
<a href="forum.php?mod=forumdisplay&amp;fid=<?php echo $_G['fid'];?>&amp;filter=lastpost&amp;orderby=lastpost<?php echo $forumdisplayadd['lastpost'];?><?php if($_GET['archiveid']) { ?>&amp;archiveid=<?php echo $_GET['archiveid'];?><?php } ?>" class="<?php if($_GET['filter'] == 'lastpost') { ?>cur<?php } ?>">æœ€æ–°</a>
</li>
<li>
<a href="forum.php?mod=forumdisplay&amp;fid=<?php echo $_G['fid'];?>&amp;filter=heat&amp;orderby=heats<?php echo $forumdisplayadd['heat'];?><?php if($_GET['archiveid']) { ?>&amp;archiveid=<?php echo $_GET['archiveid'];?><?php } ?>" class="<?php if($_GET['filter'] == 'heat') { ?>cur<?php } ?>">çƒ­é—¨</a>
</li>
<li>
<a href="forum.php?mod=forumdisplay&amp;fid=<?php echo $_G['fid'];?>&amp;filter=digest&amp;digest=1<?php echo $forumdisplayadd['digest'];?><?php if($_GET['archiveid']) { ?>&amp;archiveid=<?php echo $_GET['archiveid'];?><?php } ?>" class="<?php if($_GET['filter'] == 'digest') { ?>cur<?php } ?>">ç²¾å</a>
</li>
</ul>
</div>
...
<?php if($_G['forum_threadcount']) { ?> <!--Èç¹û Ìû×Ó×ÜÊı²»Îª false »ò 0-->
    <?php if($ceo_piclistopen) { ?> <!--Èç¹û Í¼ÎÄÁĞ±í´ò¿ª×´Ì¬-->
        <!--Èç¹û´ò¿ªÍ¼ÎÄÁĞ±í--><?php include template('forum/forumdisplay_imglist'); ?>    <?php } else { ?>
        <!--Èç¹ûÎ´´ò¿ªÍ¼ÎÄÁĞ±í--><?php include template('forum/forumdisplay_txtlist'); ?>    <?php } ?> 
<?php } else { ?>  <!--Èç¹û Ìû×Ó×ÜÊıÊÇ false »ò 0-->
<ul>
<li class="noData">æœ¬ç‰ˆå—æˆ–æŒ‡å®šçš„èŒƒå›´å†…å°šæ— ä¸»é¢˜</li>
</ul>
<?php } ?> 
</div>
<?php } ?>
